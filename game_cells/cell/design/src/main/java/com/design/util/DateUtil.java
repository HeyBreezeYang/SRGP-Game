...
public class DateUtil {
	...
	public static String getDateStringForSmsFormat(Date date) {
	...
	public static Date getLastDay(int year, int month) throws ParseException {
		Date date = getFirstDay(year,month);
	 ...
	 * @return Date
	 */
	public static int getDistinceDay(String beforedate, String afterdate) {
		SimpleDateFormat d = sdf;
		d.applyPattern("yyyy-MM-dd");
		int dayCount = 0;
		try {
			...
			dayCount = (int) ((d2.getTime() - d1.getTime()) / (24 * 60 * 60 * 1000));
		} catch (ParseException e) {
			System.out.println("Date parse error!");
		}
		return dayCount;
