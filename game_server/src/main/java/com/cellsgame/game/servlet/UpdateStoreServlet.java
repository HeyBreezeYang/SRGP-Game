...
public class UpdateStoreServlet extends AsyncServlet {

	private static final Logger log = LoggerFactory.getLogger(UpdateStoreServlet.class);
	...
	public void exec(Continuation continuation, HttpServletRequest request,
		...
		log.info("UpdateStore info : {}", info);
		try {
			getStoreBO().updateStore(info);
