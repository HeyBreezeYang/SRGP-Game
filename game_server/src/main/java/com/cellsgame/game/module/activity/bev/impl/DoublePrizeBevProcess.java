...
public class DoublePrizeBevProcess extends ABevProcess {
	...
	private static final int PrizeType_Currency = 3;
	...
	public int execBev(Map<?, ?> parent, CMD cmd, PlayerVO player, ActivityVO act,
		...
		if(sysType.check(event, type, subType)){
			...
			if(prizeType == PrizeType_GoodsID){
				Map<Integer, Integer> result = PrizeConstant.getGoods(prizeMap, 0, prizeParam);
				goodsPrizeExec(parent, cmd, result, player);
			}
			if(prizeType == PrizeType_GoodsType){
				Map<Integer, Integer> result = PrizeConstant.getGoods(prizeMap, prizeParam, 0);
				goodsPrizeExec(parent, cmd, result, player);
			}
			if(prizeType == PrizeType_Currency){
				Map<Integer, Integer> result = PrizeConstant.getCur(prizeMap, prizeParam);
				currencyPrizeExec(parent, cmd, result, player);
			}
			return 1;
		}
        return 0;
	...
	private void currencyPrizeExec(Map<?, ?> parent, CMD cmd,
			Map<Integer, Integer> result, PlayerVO player) {
		FuncsExecutor exec = FuncsExecutorsType.Base.getExecutor(cmd);
		for(Entry<Integer, Integer> entry : result.entrySet()){
			Integer curType = entry.getKey();
			Integer num = entry.getValue();
			exec.addSyncFunc(SyncFuncType.ChangeCur, curType, num);
		}
